...
export class TestGame extends Game{
  ...
  printGroups(logToConsole: boolean = false) {
    logToConsole && this.groupsHandler.groupLocations.forEach((row: Row) => console.log(row.join(" ")))
    return this.groupsHandler.groupLocations.map((row: Row) => row.join(" ")).join("\n")
  }
  ...
  simulateClick(position: Position) {
    ...
    if(this.groupsHandler.groupLocations[row][col] === "-"){
      ...
      if(!this.fixColor) this.currentColor = this.currentColor === "O" ? "X" : "O"
    }
  }
}